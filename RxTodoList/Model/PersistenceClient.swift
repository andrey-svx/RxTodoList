...
struct PersistenceClient {
    ...
    mutating func fetchTodos() -> Observable<[LocalTodo]> {
    ...
    mutating func fetchTodo(_ todo: LocalTodo) -> Observable<LocalTodo?> {
        let request = CDTodo.fetchRequest() as NSFetchRequest<CDTodo>
    ...
    mutating func removeTodo(_ todo: LocalTodo) -> Observable<Void> {
    ...
    mutating func appendTodo(_ todo: LocalTodo) -> Observable<Void> {
    ...
    mutating func editTodo(_ todo: LocalTodo, with editedTodo: LocalTodo) -> Observable<Void> {
        Observable.just(())
    }
